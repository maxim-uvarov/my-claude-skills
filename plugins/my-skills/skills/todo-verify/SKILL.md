...
A `todo/` note is a problem written down to be dealt with later. Most of them are the user's own; some were written by an agent, because he asked for one. Notes accumulate, get marked closed by whoever passed by, and that marker is a claim like any other — added without re-running anything. This skill turns the folder back into a backlog you can plan from: every closure claim is re-tested against the current tree, and a note leaves the folder only when a command proves it.

Who wrote a note does not change how it is handled — check it, verify it, then move it or keep it, the same either way. It changes only how carefully the file itself is treated: most of this folder is the user's own writing, and it is not yours to rephrase.
...
Never use `git checkout`, `git switch`, or `git reset --hard` to reach an older state. Where the repo leaves `todo/` uncommitted — which is every published repo — a checkout destroys the notes outright, and it has silently destroyed uncommitted work here before. In an internal monorepo the notes are tracked and survive, but the same command still takes every other uncommitted change with it. Use `git worktree add` into the scratchpad, or copy the tree.
...
Why move and not delete: the verdict is yours, not the user's, and a wrong CONFIRMED FIXED would erase the evidence that would have shown it wrong. In a published repo the notes are also uncommitted, so `rm` is unrecoverable — git will not bring them back. Emptying `todo/verified/` is then one command, on the user's own schedule.
...
- **The note describes a fix that is now wrong for a different reason.** That is a new problem, not this note's. Report it in the run's output and leave the old note alone; write a new note only if the user asks for one.
